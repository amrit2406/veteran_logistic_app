...
namespace veteran_logistic.Reports.UnloadingReport.Export.Excel;

/// <summary>
/// Implementation of the unloading report Excel exporter.
/// </summary>
public sealed class UnloadingReportExcelExporter : IUnloadingReportExcelExporter
{
    private readonly ILogger<UnloadingReportExcelExporter> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="UnloadingReportExcelExporter"/> class.
    /// </summary>
    /// <param name="logger">The logger.</param>
    public UnloadingReportExcelExporter(ILogger<UnloadingReportExcelExporter> logger)
    {
    ...
    {
        _logger.LogInformation("Starting Excel export for unloading report to {FilePath}", filePath);

        using var workbook = new XLWorkbook();
        var worksheet = workbook.Worksheets.Add("Unloading Report");
        ...
        worksheet.Cell("A2").Value = "Unloading Report";
        ...
        worksheet.Cell($"B{headerRow}").Value = "Unloading Date";
        ...
        worksheet.Cell($"M{headerRow}").Value = "Unloading Weight";
        worksheet.Cell($"N{headerRow}").Value = "Shortage Weight";
        worksheet.Cell($"O{headerRow}").Value = "Rate";
        worksheet.Cell($"P{headerRow}").Value = "Gross Amount";
        worksheet.Cell($"Q{headerRow}").Value = "Fuel Amount";
        worksheet.Cell($"R{headerRow}").Value = "Cash Advance";
        worksheet.Cell($"S{headerRow}").Value = "Other Advance";
        worksheet.Cell($"T{headerRow}").Value = "Payment Location";
        worksheet.Cell($"U{headerRow}").Value = "Union/Vendor";
        worksheet.Cell($"V{headerRow}").Value = "Owner";
        worksheet.Cell($"W{headerRow}").Value = "Active";

        var headerRange = worksheet.Range($"A{headerRow}:W{headerRow}");
        ...
        foreach (var item in items)
        {
            ...
            worksheet.Cell($"B{dataRow}").Value = item.UnloadingDate;
            ...
            worksheet.Cell($"M{dataRow}").Value = item.UnloadingWeight;
            worksheet.Cell($"M{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"N{dataRow}").Value = item.ShortageWeight;
            worksheet.Cell($"N{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"O{dataRow}").Value = item.Rate;
            worksheet.Cell($"O{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"P{dataRow}").Value = item.GrossAmount;
            worksheet.Cell($"P{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"Q{dataRow}").Value = item.FuelAmount;
            worksheet.Cell($"Q{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"R{dataRow}").Value = item.CashAdvance;
            worksheet.Cell($"R{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"S{dataRow}").Value = item.OtherAdvance;
            worksheet.Cell($"S{dataRow}").Style.NumberFormat.Format = "#,##0.00";
            worksheet.Cell($"T{dataRow}").Value = item.PaymentLocationName;
            worksheet.Cell($"U{dataRow}").Value = item.UnionVendorName;
            worksheet.Cell($"V{dataRow}").Value = item.OwnerName;
            worksheet.Cell($"W{dataRow}").Value = item.IsActive ? "Yes" : "No";

            dataRow++;
        }

        // Data borders
        var dataRange = worksheet.Range($"A{headerRow}:W{dataRow - 1}");
        ...
        worksheet.Cell($"A{totalsRow + 4}").Value = "Total Unloading Weight:";
        worksheet.Cell($"B{totalsRow + 4}").Value = totals.TotalUnloadingWeight;
        worksheet.Cell($"B{totalsRow + 4}").Style.NumberFormat.Format = "#,##0.00";

        worksheet.Cell($"A{totalsRow + 5}").Value = "Total Shortage Weight:";
        worksheet.Cell($"B{totalsRow + 5}").Value = totals.TotalShortageWeight;
        worksheet.Cell($"B{totalsRow + 5}").Style.NumberFormat.Format = "#,##0.00";

        worksheet.Cell($"A{totalsRow + 6}").Value = "Total Gross Amount:";
        worksheet.Cell($"B{totalsRow + 6}").Value = totals.TotalGrossAmount;
        worksheet.Cell($"B{totalsRow + 6}").Style.NumberFormat.Format = "#,##0.00";

        worksheet.Cell($"A{totalsRow + 7}").Value = "Total Fuel Amount:";
        worksheet.Cell($"B{totalsRow + 7}").Value = totals.TotalFuelAmount;
        worksheet.Cell($"B{totalsRow + 7}").Style.NumberFormat.Format = "#,##0.00";

        worksheet.Cell($"A{totalsRow + 8}").Value = "Total Cash Advance:";
        worksheet.Cell($"B{totalsRow + 8}").Value = totals.TotalCashAdvance;
        worksheet.Cell($"B{totalsRow + 8}").Style.NumberFormat.Format = "#,##0.00";

        worksheet.Cell($"A{totalsRow + 9}").Value = "Total Other Advance:";
        worksheet.Cell($"B{totalsRow + 9}").Value = totals.TotalOtherAdvance;
