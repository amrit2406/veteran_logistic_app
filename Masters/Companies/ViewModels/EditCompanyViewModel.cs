...
/// <summary>
/// ViewModel for the Edit Company screen.
/// </summary>
public sealed partial class EditCompanyViewModel : ViewModelBase, INavigationAware
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EditCompanyViewModel"/> class.
    /// </summary>
    ...
    public EditCompanyViewModel(ICompanyCommandService companyCommandService, ICompanyQueryService companyQueryService, INavigationService navigationService)
    {
    ...
    {
        if (parameter is not null && parameter.TryGetValue<int>("CompanyId", out var companyId))
        {
    ...
    /// </summary>
    private async Task LoadCompanyAsync(CancellationToken cancellationToken = default)
    {
        if (_companyId == 0)
        {
            ValidationError = "Company ID is required.";
            return;
        }

        SetBusy("Loading company...");
        var company = await _companyQueryService.GetCompanyForEditAsync(_companyId, cancellationToken).ConfigureAwait(false);
        ClearBusy();
    ...
    private async Task SaveAsync()
    {
        ...
        SetBusy("Updating company...");
        var result = await _companyCommandService.UpdateCompanyAsync(request, CancellationToken.None).ConfigureAwait(false);
        ClearBusy();
        ...
        {
            ValidationError = result.ErrorMessage ?? "Failed to update company.";
        }
    }
