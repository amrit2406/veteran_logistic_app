...
/// <summary>
/// ViewModel for the Edit HSD Rate screen.
/// </summary>
public sealed partial class EditHsdRateViewModel : ViewModelBase, INavigationAware
{
    ...
    private int _hsdRateId;
    private int _fuelPumpId;
    private DateTime _applicableDate;
    private decimal _ratePerLitre;
    private bool _isActive;
    private string _validationError = string.Empty;

    /// <summary>
    /// Initializes a new instance of the <see cref="EditHsdRateViewModel"/> class.
    /// </summary>
    ...
    public EditHsdRateViewModel(IHsdRateCommandService hsdRateCommandService, IHsdRateQueryService hsdRateQueryService, INavigationService navigationService)
    {
    ...
    {
        if (parameter is not null && parameter.TryGetValue<int>("HsdRateId", out var hsdRateId))
        {
            _hsdRateId = hsdRateId;
        }
    }
    ...
    public override async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        await LoadHsdRateAsync(cancellationToken);
    ...
    /// <summary>
    /// Loads the HSD rate data for editing.
    /// </summary>
    private async Task LoadHsdRateAsync(CancellationToken cancellationToken = default)
    {
        if (_hsdRateId == 0)
        {
            ValidationError = "HSD rate ID is required.";
            return;
        }

        SetBusy("Loading HSD rate...");
        var hsdRate = await _hsdRateQueryService.GetHsdRateForEditAsync(_hsdRateId, cancellationToken).ConfigureAwait(false);
        ClearBusy();

        if (hsdRate is null)
        {
            ValidationError = "HSD rate not found.";
            return;
        }

        FuelPumpId = hsdRate.FuelPumpId;
        ApplicableDate = hsdRate.ApplicableDate;
        RatePerLitre = hsdRate.RatePerLitre;
        IsActive = hsdRate.IsActive;
    }
    ...
    private async Task SaveAsync()
    {
        ...
        var request = new UpdateHsdRateRequest
        {
            HsdRateId = _hsdRateId,
        ...
        SetBusy("Updating HSD rate...");
        var result = await _hsdRateCommandService.UpdateHsdRateAsync(request, CancellationToken.None).ConfigureAwait(false);
        ClearBusy();
        ...
        {
            ValidationError = result.ErrorMessage ?? "Failed to update HSD rate.";
        }
    }
